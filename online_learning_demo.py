...
training_start = 0
training_end = 1000
surrogate_learning_threshold = 2
start_time = 0
end_time = 500
plot_length = 20
spin_up = 20
...
if __name__ == "__main__":
    ...
    while current_time <= end_time - plot_length:
        ...
        for i in range(int(plot_length / dt)):
            ...
            (next_state, cov) = surrogate_gpr.predict([current_state], return_cov=True)
            next_state = next_state[0]
            print(" std ", std, " cov ", cov)
            if std > surrogate_learning_threshold or learning_time <= spin_up:
